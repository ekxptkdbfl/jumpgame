...
// --- [스폰 시스템 변수] ---
let spawnTimer = 0;
let nextSpawnTime = 150;
let longGapEnforceCount = 0; 

let fastEventActive = false;
let fastEventTimer = 0;
const EVENT_DURATION = 360; 
let fastCarSpawnFrame = 0;
let lastFastMilestone = 0;
...
let penguin = { x: 0, y: 0, width: 45, height: 45, dy: 0, jumpPower: 6, gravity: 0.9, floatGravity: 0.15, onRoad: true };
...
class Car {
    constructor(isFast = false) {
        this.carWidth = isFast ? 100 : (Math.random() > 0.5 ? 130 : 80);
        this.height = 40; 
...
// --- [수정 포인트: 도로 속도 시각적 하향 (30% 느리게)] ---
function updateEnvironment() {
    if (isPaused) return;
    // 자동차 속도의 70% 수준으로만 오프셋을 이동시켜 패럴랙스 효과 구현
...
function drawRoadDetails() {
    ...
    const centerY = groundY + roadHeight * 0.45;
    ctx.save();
    ctx.strokeStyle = 'rgba(255, 255, 255, 0.5)';
    ctx.lineWidth = 12; 
    ctx.setLineDash([40, 40]); 
    ctx.beginPath();
    ctx.moveTo(-roadOffset, centerY);
...
function drawBackground(env) {
    ...
    buildings.forEach(b => {
        if (b.type === 'building') {
            ...
            // --- [수정 포인트: 낮 나무 색상 원래대로, 밤 나무만 어둡게 보간] ---
            const DAY_LEAF = [39, 174, 96];   // 원래 색상 (#27ae60)
            const NIGHT_LEAF = [10, 40, 20];   // 밤의 어두운 나뭇잎
            const DAY_TRUNK = [141, 110, 99]; // 원래 나무 기둥 (#8D6E63)
            const NIGHT_TRUNK = [98, 77, 69];  // 밤의 어두운 기둥 (#624D45)
            
            ctx.fillStyle = `rgb(${lerpColor(DAY_TRUNK, NIGHT_TRUNK, env.lightOpacity).join(',')})`; 
            ctx.fillRect(b.x, groundY - b.height, b.width, b.height);
            ctx.fillStyle = `rgb(${lerpColor(DAY_LEAF, NIGHT_LEAF, env.lightOpacity).join(',')})`;
            ctx.fillRect(b.x - 20, groundY - b.height - 30, b.width + 40, 40);
        } else if (b.type === 'crosswalk') {
            ctx.fillStyle = 'rgba(255, 255, 255, 0.5)';
            for(let i=0; i<b.stripes; i++) ctx.fillRect(b.x + i * 25, groundY + 5, 12, 30);
        }
    });
}
...
function updateCity() {
    ...
    const groundY = getGroundY() - penguin.height;
    if (keys['Space'] && penguin.onRoad) { penguin.dy = -penguin.jumpPower; penguin.onRoad = false; wasInAir = true; }
    let curGrav = (!penguin.onRoad && keys['Space']) ? penguin.floatGravity : penguin.gravity;
    penguin.dy += curGrav; penguin.y += penguin.dy;
    if (penguin.y > groundY) {
        penguin.y = groundY; penguin.dy = 0; penguin.onRoad = true;
        if (wasInAir) { score += 5; wasInAir = false; }
    }
    let currentMilestone = Math.floor(score / 500);
    if (currentMilestone > lastFastMilestone && !fastEventActive) {
        fastEventActive = true; lastFastMilestone = currentMilestone; fastEventTimer = 0;
        fastCarSpawnFrame = Math.floor(Math.random() * 60) + 300; 
    }
    if (fastEventActive) {
        fastEventTimer++;
        if (fastEventTimer === fastCarSpawnFrame) cars.push(new Car(true));
    ...
    cars.forEach((car, index) => {
        ...
        // --- [수정 포인트: 장애물 통과 점수 5점으로 하향] ---
        if (!car.passed && car.x + car.carWidth < penguin.x) { 
            car.passed = true; score += 5; obstaclesPassed++; 
            if (obstaclesPassed % 20 === 0) citySpeed *= 1.1; 
        }
        if (car.x + car.carWidth < 0) cars.splice(index, 1);
        if (penguin.x < (car.x + car.carWidth) - 15 && penguin.x + penguin.width > car.x + 15 && penguin.y + penguin.height > car.y + 10) {
...
function animate() {
    ...
    drawRoadDetails();
    if (isGameStarted) {
        if (!isGameOver) { updateCity(); drawPenguin(); cars.forEach(c => c.draw(env)); }
        ctx.save();
